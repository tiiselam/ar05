...
/****** Object:  View [dbo].[GXPR_FC_ELECTRONICA]    Script Date: 9/28/2018 5:09:30 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [dbo].[GXPR_FC_ELECTRONICA]
AS
SELECT        /*Datos Compañia Emite*/ rtrim(CMP.CMPNYNAM) CMPNYNAM/*OK*/ , rtrim(concat(rtrim(CMP.ADDRESS1), rtrim(CMP.ADDRESS2), rtrim(CMP.ADDRESS3), ' - ', rtrim(CMP.CITY), ' - ', rtrim(CMP.STATE))) 
                         CMPADRESS/*OK*/ , 'file:' + ARC.fileName FileName/*OK*/ , Substring(TAXREGTN, 1, 2) + '-' + Substring(TAXREGTN, 3, 8) + '-' + Substring(TAXREGTN, 11, 1) CUIT/*OK Falta separar*/ , Substring(INFO.INETINFO, 
                         ...
                         rtrim(DIR.CITY), ' - ', rtrim(DIR.STATE))) CLNT_ADRESS/* DAtos FACnte Factura*/ , rtrim(FAC.CURNCYID) CURNCYID, FAC.XCHGRATE, FAC.ORTDISAM TRDISAMT/*OK*/ , FAC.ORSUBTOT SUBTOTAL/*OK*/ , 
                         ...
                         Substring(FAC.TXRGNNUM, 1, 2) + '-' + Substring(FAC.TXRGNNUM, 3, 8) + '-' + Substring(FAC.TXRGNNUM, 11, 1) CLNT_CUIT/*OK*/ , IVA.RESPBLE CLNT_IVA/*OK*/ , rtrim(FORMAT(FAC.DUEDATE, 'dd-MM-yyyy')) 
                         ...
                         'V-IMPMUN%') IMP_MUNICIPAL/* Detalles Factura*/ , rtrim([ITEMNMBR]) ITEMNMBR, rtrim(ITEMDESC) ITEMDESC/*OK*/ , rtrim([UOFM]) UOFM/*OK*/ , ORUNTPRC UNITPRCE/*OK*/ , OXTNDPRC XTNDPRCE/*OK*/ , 
                         DET.ORTAXAMT TAXAMNT/*,convert(decimal(10,2),PORC_IVA) --'21' PORC_IVA*/ , dbo.GXPRFuncGetTaxItm(DET.SOPTYPE, DET.SOPNUMBE, DET.LNITMSEQ) PORC_IVA, DET.ORMRKDAM MRKDNAMT/*OK */ , 
                         QUANTITY QUANTITY
/*OK*/ FROM DYNAMICS.dbo.SY01500 AS CMP INNER JOIN
                         ...
                         SOP10105 IMP ON DET.SOPTYPE = IMP.SOPTYPE AND DET.SOPNUMBE = IMP.SOPNUMBE AND DET.LNITMSEQ = IMP.LNITMSEQ AND IMP.TAXDTLID LIKE '%' + rtrim('V-IV-') + '%'
...
SELECT        /*Datos Compañia Emite*/ rtrim(CMP.CMPNYNAM) CMPNYNAM/*OK*/ , rtrim(concat(rtrim(CMP.ADDRESS1), rtrim(CMP.ADDRESS2), rtrim(CMP.ADDRESS3), ' - ', rtrim(CMP.CITY), ' - ', rtrim(CMP.STATE))) 
                         ...
                         DET.ORMRKDAM MRKDNAMT/*OK */ , QUANTITY QUANTITY
/*OK*/ FROM DYNAMICS.dbo.SY01500 AS CMP INNER JOIN
                         DYNAMICS.dbo.SY01501 AS ARC ON ARC.CMPANYID = CMP.CMPANYID LEFT OUTER JOIN
                         ...
                         FEI_SOP30200 AS FEI ON FAC.SOPTYPE = FEI.SOPTYPE AND FAC.SOPNUMBE = FEI.SOPNUMBE LEFT OUTER JOIN
                         AWLI_RM00101 AS RM ON FAC.CUSTNMBR = RM.CUSTNMBR LEFT OUTER JOIN
                         DYNAMICS.dbo.AWLI40330 AS IVA ON RM.RESP_TYPE = IVA.RESP_TYPE
WHERE        CMP.INTERID = DB_NAME() AND INFO.ADRSCODE = 'FC_ELECTRONICA' AND INFO.MASTER_TYPE = 'CMP'
GO
...
Begin DesignProperties = 
   Begin PaneConfigurations = 
      Begin PaneConfiguration = 0
         NumPanes = 4
         Configuration = "(H (1[21] 4[6] 2[54] 3) )"
      End
   ...
   End
End
' , @level0type=N'SCHEMA',@level0name=N'dbo', @level1type=N'VIEW',@level1name=N'GXPR_FC_ELECTRONICA'
GO

EXEC sys.sp_addextendedproperty @name=N'MS_DiagramPaneCount', @value=1 , @level0type=N'SCHEMA',@level0name=N'dbo', @level1type=N'VIEW',@level1name=N'GXPR_FC_ELECTRONICA'
GO
