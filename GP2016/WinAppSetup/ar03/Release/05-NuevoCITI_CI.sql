...
/****** Object:  StoredProcedure [dbo].[NuevoCITI_CI]    Script Date: 21/08/2018 14:57:49 ******/
SET ANSI_NULLS ON
GO
...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[NuevoCITI_CI]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[NuevoCITI_CI]
go
...
INSERT INTO #TEMP2
SELECT A.VCHRNMBR, A.DOCTYPE, A.VENDORID, A.COMPTYPE, A.PDV, A.NROCOMP, A.DESPACHO, A.CODDOC, A.NROIDENT,
...
where A.rptid IN(@REPORTE)	 AND CI.TipoReporte = 1 AND CI.ColumnaArchivo = 2 ) TX ON B.TAXDTLID = TX.TAXDTLID
WHERE TX.ColumnaArchivo = 2 AND A.CODOPER IN(1, 2, 3) AND B.TAXAMNT > 0 AND A.ALICUOTAS <> 0
GROUP BY A.VCHRNMBR, A.DOCTYPE, A.VENDORID, A.COMPTYPE, A.PDV, A.NROCOMP, A.DESPACHO, A.CODDOC, A.NROIDENT, TX.TXDTLPCT
...
SELECT 
A.DESPACHO							+		-- DESPACHO
RIGHT(REPLACE('000000000000000' + LEFT(CONVERT(VARCHAR, GRAVADO), LEN(CONVERT(VARCHAR, GRAVADO))-3), '.', ''), 15)   + -- GRAVADO
...
WHERE LEN(REPLACE(A.DESPACHO, '  ', '')) = 16 AND A.DESPACHO <> ''
